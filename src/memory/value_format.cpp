                 ...
                 size_t sz, bool utf16)
{
    ...
    case mem::ValueType::String: {
        if (utf16)
        {
            // Up to len/2 UTF-16LE units (stop at NUL), re-encoded to UTF-8. A
            // lone surrogate from truncation renders as a replacement glyph.
            const size_t units = len / 2;
            std::vector<wchar_t> w(units ? units : 1);
            memcpy(w.data(), buf, units * 2);
            size_t wlen = 0;
            while (wlen < units && w[wlen]) ++wlen;
            int n = wlen ? WideCharToMultiByte(CP_UTF8, 0, w.data(), (int)wlen, out,
                                               (int)sz - 1, nullptr, nullptr)
                         : 0;
            if (n < 0) n = 0;
            out[n] = '\0';
            break;
